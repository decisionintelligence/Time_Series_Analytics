...
class TCN(nn.Module):
    def __init__(self, input_dim, num_channels, kernel_size=2, dropout=0.2):
        """
        Temporal Convolutional Network (TCN) 的简单实现
        - input_dim: 输入变量的数量
        ...
        for i in range(num_layers):
            ...
            # 定义一层带因果卷积的残差块
            layers.append(ResidualBlock(in_channels, out_channels, kernel_size, stride=1, dropout=dropout))
    ...
    def forward(self, x):
        ...
        - 输出: (batch_size, seq_len, num_channels[-1])
        """
        # 转换为 (batch_size, input_dim, seq_len) 以适配 1D 卷积
        x = x.transpose(1, 2)
        x = self.network(x)
        # 转换回 (batch_size, seq_len, num_channels[-1])
        return x.transpose(1, 2)

class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, dropout):
        super(ResidualBlock, self).__init__()
        self.conv1 = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride, padding=kernel_size - 1)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout)
        self.conv2 = nn.Conv1d(out_channels, out_channels, kernel_size, stride=stride, padding=kernel_size - 1)
        self.downsample = nn.Conv1d(in_channels, out_channels, kernel_size=1) if in_channels != out_channels else None

    def forward(self, x):
        residual = x
        x = self.conv1(x)[:, :, :x.size(2)]  # 修正时间步
        x = self.relu(x)
        x = self.dropout(x)
        x = self.conv2(x)[:, :, :x.size(2)]  # 修正时间步
        if self.downsample is not None:
            residual = self.downsample(residual)
        # 如果残差形状不一致，进行填充
        if residual.size(2) != x.size(2):
            diff = x.size(2) - residual.size(2)
...
# 定义 TCN 模型
num_channels = [32, 64, 7]   # 每一层的输出通道数
  
model = TCN(input_dim=input_dim, num_channels=num_channels)
